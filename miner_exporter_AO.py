...
api = appoptics_metrics.connect(os.environ['AO_API'])
q = api.new_queue()
...
HNT_CHAIN = os.environ.get('HNT_CHAIN', 'main')
...
def stats():
  ...
  miner_version = collect_miner_version(docker_container, hotspot_name_str) 

  # collect total cpu and memory usage. Might want to consider just the docker
  # container with something like cadvisor instead
  q.add('hnt.system_usage.cpu', psutil.cpu_percent(), tags={'hotspot': hotspot_name_str, 'version': miner_version})
  q.add('hnt.system_usage.memory',  psutil.virtual_memory()[2], tags={'hotspot': hotspot_name_str, 'version': miner_version})
  #q.add('hnt.system_usage.cpu.steal',  psutil.cpu_times_percent().steal, tags={'hotspot': hotspot_name_str})
  q.add('hnt.system_usage.cpu.user',  psutil.cpu_times_percent().user, tags={'hotspot': hotspot_name_str, 'version': miner_version})
  q.add('hnt.system_usage.cpu.system',  psutil.cpu_times_percent().system, tags={'hotspot': hotspot_name_str, 'version': miner_version})
  q.add('hnt.system_usage.cpu.idle',  psutil.cpu_times_percent().idle, tags={'hotspot': hotspot_name_str, 'version': miner_version})
  q.add('hnt.system_usage.disk.used',  float(psutil.disk_usage('/').total), tags={'hotspot': hotspot_name_str, 'version': miner_version})
  q.add('hnt.system_usage.disk.used',  psutil.virtual_memory()[2], tags={'hotspot': hotspot_name_str, 'version': miner_version})
  q.add('hnt.system_usage.process.count', (sum(1 for proc in psutil.process_iter())), tags={'hotspot': hotspot_name_str, 'version': miner_version})
...
def collect_container_run_time(docker_container, miner_name):
  ...
  if attrs:
    if attrs.get("Created"):
      ...
      q.add('hnt.uptime.create', create_delta, tags={'hotspot': miner_name})
    if attrs.get("State") and attrs["State"].get("StartedAt"):
      ...
      q.add('hnt.uptime.start', start_delta, tags={'hotspot': miner_name})
...
def collect_chain_stats():
  ...
  q.add('hnt.chain_stats.height', height_val, tags={'chain': f'{HNT_CHAIN}'})
  ...
  q.add('hnt.chain_stats.staked_validators', count_val, tags={'chain': f'{HNT_CHAIN}'})
...
def collect_balance(docker_container, addr, miner_name):
  ...
  q.add('hnt.balance', balance, tags={'hotspot': miner_name})
...
def collect_miner_height(docker_container, miner_name):
  ...
  log.debug(out.output)
  log.info(out.output.split()[1])
  txt = out.output.decode('utf-8').rstrip("\n")
  hnt_height = txt.split()  # list of words in the string
  log.info(hnt_height[-1])

  txt = out.output.decode('utf-8').rstrip("\n")
  

  q.add('hnt.miner.height', hnt_height[-1], tags={'hotspot': miner_name})
...
def collect_in_consensus(docker_container, miner_name):
  ...
  q.add('hnt.miner.incon', incon, tags={'hotspot': miner_name})
...
def collect_block_age(docker_container, miner_name):
  ...
  q.add('hnt.blockage', age_val, tags={'hotspot': miner_name})
...
def collect_hbbft_performance(docker_container, miner_name):
  ...
  for line in out.output.decode('utf-8').split("\n"):
    ...
    # always set these, that way they get reset when out of CG3

    q.add('hnt.hbbft_perf.penalty', hval.get('pen_val', 0), tags={'hotspot': miner_name})
    #HBBFT_PERF.labels('hbbft_perf','Penalty', miner_name).set(hval.get('pen_val', 0))

    q.add('hnt.hbbft_perf.hbbft_perf', hval.get('pen_val', 0), tags={'hotspot': miner_name})
    #HBBFT_PERF.labels('hbbft_perf','BBA_Total', miner_name).set(hval.get('bba_tot', 0))

    q.add('hnt.hbbft_perf.BBA_votes', hval.get('bba_votes', 0), tags={'hotspot': miner_name})
    #HBBFT_PERF.labels('hbbft_perf','BBA_Votes', miner_name).set(hval.get('bba_votes', 0))

    q.add('hnt.hbbft_perf.seen_total', hval.get('seen_tot', 0), tags={'hotspot': miner_name})
    #HBBFT_PERF.labels('hbbft_perf','Seen_Total', miner_name).set(hval.get('seen_tot', 0))

    q.add('hnt.hbbft_perf.seen_votes', hval.get('seen_votes', 0), tags={'hotspot': miner_name})
    # HBBFT_PERF.labels('hbbft_perf','Seen_Votes', miner_name).set(hval.get('seen_votes', 0))

    q.add('hnt.hbbft_perf.BBA_last', hval.get('bba_last_val', 0), tags={'hotspot': miner_name})
    #HBBFT_PERF.labels('hbbft_perf','BBA_Last', miner_name).set(hval.get('bba_last_val', 0))

    q.add('hnt.hbbft_perf.seen_last', hval.get('seen_last_val', 0), tags={'hotspot': miner_name})
    #HBBFT_PERF.labels('hbbft_perf','Seen_Last', miner_name).set(hval.get('seen_last_val', 0))
    
    q.add('hnt.hbbft_perf.tenure', hval.get('tenure', 0), tags={'hotspot': miner_name})
...
def collect_peer_book(docker_container, miner_name):
  ...
  for line in out.output.decode('utf-8').split("\r\n"):
    ...
    if len(c) == 6:
      ...
      if miner_name == peer_name and isinstance(conns_num, int):
        q.add('hnt.connections', conns_num, tags={'hotspot': miner_name})
  ...
  q.add('hnt.sessions', sessions, tags={'hotspot': miner_name})
  ...
  for line in [x.rstrip("\r\n") for x in results]:
    ...
    if len(c) == 10:
      ...
      if miner_name == val_name:
        ...
        q.add('hnt.ledger_penalties.tenure', tenure_penalty_val, tags={'hotspot': miner_name})

        #LEDGER_PENALTY.labels('ledger_penalties', 'dkg', miner_name).set(dkg_penalty_val)
        q.add('hnt.ledger_penalties.dkg', dkg_penalty_val, tags={'hotspot': miner_name})

        #LEDGER_PENALTY.labels('ledger_penalties', 'performance', miner_name).set(performance_penalty_val)
        q.add('hnt.ledger_penalties.performance', performance_penalty_val, tags={'hotspot': miner_name})

        #LEDGER_PENALTY.labels('ledger_penalties', 'total', miner_name).set(total_penalty_val)
        q.add('hnt.ledger_penalties.BBA_last', total_penalty_val, tags={'hotspot': miner_name})
        
        # Blockage
        q.add('hnt.BLOCKAGE.last_heartbeat', last_heartbeat, tags={'hotspot': miner_name})
...
def collect_miner_version(docker_container, miner_name):
  ...
  for line in results:
    if m := re.match('^\*\s+([\d\.]+)(.*)', line):
      ...
      # return miner_version to be used in Tags. 

if __name__ == '__main__':
 # Submit metrics to AppOptics
  log.info("Submitting metrics to AppOptics")
  while True: 
